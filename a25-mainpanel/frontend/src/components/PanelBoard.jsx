...
export default function PanelBoard({
  ...
  const [opacityGrid, setOpacityGrid] = useState(
    new Array(grid.length).fill().map(() => new Array(grid[0].length).fill(1))
  );
  const handlePanelOpacity = (i, j) => {
    let newOpacityGrid = [...opacityGrid];
    if (panelOpacity) {
  ...
  let finalQuizMusic = new Audio();
  let finalQuestionVoice = new Audio();
  const seikaiwa = new Audio("seikaiwa000.wav");
  let explanationVoice = new Audio();
  ...
  let count = 0;
  let finalAnswerVoice = new Audio();
